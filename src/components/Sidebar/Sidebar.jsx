...
const { setIsLoggedIn } = useContext(AuthenticationContext);
    ...
    { currentUser.avatar? (<img className='Sidebar__avatar' src={currentUser.avatar} alt="logo" />) : (<div className="Sidebar__avatar-placeholder">{ currentUser.name ? (<p> {currentUser.name[0].toUpperCase()} </p>) : (<img
              src={defaultAvatar}
     ...
     <span className='Sidebar__username'>{currentUser?.name }</span>
      ...
      <p className="Sidebar__button" onClick={() => {
        handleEditProfileClick();
      }}>Change Profile data</p>
      <p className="Sidebar__button" onClick={() => {
      removeToken();
      setIsLoggedIn(false);
